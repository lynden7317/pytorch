...
def mask_decode(image, result, classes, outputfolder, tag='c'):
    ...
    for _m in range(masks.shape[0]):
        ...
        if tag == 'd':
            ...
            pos = np.where(masks[_m, :, :] > 0.5)
            print(pos[0].shape[0], classes[lab])
        ...
        pos_img = np.zeros((img_bgr.shape[0], img_bgr.shape[0]))
        _name = img_name+"_" + tag + "_"+str(lab)+"_"+str(len(cls_list[lab]))+".jpg"

        pos_img[:, :] = np.where(masks[_m, :, :] > 0.5, 1, 0)
        for c in range(3):
        ...
        cls_list[lab].append((path_org, path, (x1,y1), (w,h), mask_img, pos_img))
...
def merge_car_damage_segs(cars, damages):
    print(cars.keys(), damages.keys())
    for _c in cars.keys():
        if _c in ["CC", "CP", "CL"]:
            continue
        else:
...
def car_segmentation(cases, folder):
    ...
    for images, targets in metric_logger.log_every(dbloader, dbloader.batch_size, header):
        ...
        car_mask_dict, damage_mask_dict = None, None
        for _i, c in enumerate(c_outputs):
            ...
            if len(c['labels']) > 0:
                ...
                for _k in car_mask_dict.keys():
                    car_class_dict[_k] += car_mask_dict[_k]

            if car_mask_dict is not None and damage_mask_dict is not None:
                logging.debug("merge car & damage segmentations")
                merge_car_damage_segs(car_mask_dict, damage_mask_dict)
...
def color_detection(case_list):
    ...
    for c in case_list:
        ...
        #print(x, y)
        mask_img = c[4][y1:y1 + h, x1:x1 + w, :]
        pos_mask = c[5][y1:y1+h, x1:x1+w]
